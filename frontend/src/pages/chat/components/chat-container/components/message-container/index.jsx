...
const MessageContainer = () => {
    ...
    useEffect(() => {
        const getMessages = async () => {
            try{
                const res = await apiClient.post(GET_ALL_MESSAGES_ROUTE, {id: selectedChatData.id}, {withCredentials: true})
        ...
        const getChannelMessages = async ()=>{
            try{
                const res = await apiClient.get(
                    `${GET_CHANNEL_MESSAGES_ROUTE}/${selectedChatData.id}`, {withCredentials: true}
                )
                console.log("chatId: ", selectedChatData.id)
                console.log("channelMessages: ", res.data.messages)
    ...
    const renderDMMessages = (message) => <div className={`${
        message.senderId === selectedChatData.id ? "text-left" : "text-right"
    }`}>
        {
            message.messageType === "text" && <div className={`${message.senderId !== selectedChatData.id 
            ? "bg-[#8417ff]/5 text-[#8417ff]/90 border-[#8417ff]/50" 
            : "bg-[#2a2b33]/5 text-white/80 border-[#ffffff]/20"
        } border inline-block p-4 rounded my-1 max-w-[50%] break-words`}>{message.content}</div>

        }
        {
            message.messageType === "file" && <div className={`${message.senderId !== selectedChatData.id 
                ? "bg-[#8417ff]/5 text-[#8417ff]/90 border-[#8417ff]/50" 
                : "bg-[#2a2b33]/5 text-white/80 border-[#ffffff]/20"
    ...
    const renderChannelMessages = (message) => {
        return (
            ...
            >
                {
            message.messageType === "text" && <div className={`${message.sender.id === userInfo.id 
            ? "bg-[#8417ff]/5 text-[#8417ff]/90 border-[#8417ff]/50" 
            : "bg-[#2a2b33]/5 text-white/80 border-[#ffffff]/20"
        } border inline-block p-4 rounded my-1 max-w-[50%] break-words ml-9`}>{message.content}</div>

        }

{
            message.messageType === "file" && <div className={`${message.sender.id === userInfo.id 
                ? "bg-[#8417ff]/5 text-[#8417ff]/90 border-[#8417ff]/50" 
                : "bg-[#2a2b33]/5 text-white/80 border-[#ffffff]/20"
            } border inline-block p-4 rounded my-1 max-w-[50%] break-words`}>
                {checkIfImage(message.fileUrl) ? 
        ...
        {
            message.sender.id !== userInfo.id ? <div className="flex items-center justify-start gap-3">

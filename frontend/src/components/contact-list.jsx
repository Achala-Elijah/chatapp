...
const ContactList = ({contacts, isChannel = false}) => {
    ...
    return <div className="mt-5">
        {contacts.map((contact) => (<div key={contact.id} className={`pl-10 py-2 transition-all duration-300 cursor-pointer ${selectedChatData && selectedChatData.id === contact.id ? "bg-[#8417ff] hover:bg-[#8417ff]" : "bg-[#f1f1f111]"}`}
            ...
            <div className="flex gap-5 items-center justify-start text-neutral-300">
                {
                    !isChannel && <Avatar className="h-10 w-10 rounded-full overflow-hidden">
                    {
                        ...
                        : 
                        <div className={`
                        ${selectedChatData && selectedChatData.id === contact.id ? "bg-[#ffffff22] border border-white/70" : getColor(contact.color)}
